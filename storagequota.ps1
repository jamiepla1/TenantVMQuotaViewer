# PowerShell Script to Get Storage Quota Consumption Across All Subscriptions in a Tenant
...
try {
    ...
    Write-Error "Failed to import required Az modules. Please ensure the Az module is installed: Install-Module -Name Az.Accounts, Az.Storage -Scope CurrentUser"
    exit 1
}
...
foreach ($subscription in $subscriptions) {
    ...
    # Get Storage quota usage for UK South
    try {
        ...
        if ($storageUsageData.StatusCode -eq 200) {
            ...
            if ($usages) {
                ...
                Write-Host "  No Storage quota data found for $targetLocation" -ForegroundColor Yellow
            }
        }
        elseif ($storageUsageData.StatusCode -eq 400) {
            Write-Warning "Bad request for subscription $($subscription.Name). This might indicate the subscription doesn't have Storage registered or the location is invalid."
        }
        else {
            Write-Warning "API request failed with status code: $($storageUsageData.StatusCode) for subscription $($subscription.Name)"
        }
    }
    catch {
...
# $quotaResults | Export-Csv -Path "StorageQuotaReport.csv" -NoTypeInformation
...
<head>
    ...
    <title>Azure Storage Quota Report - Tenant Wide Summary</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔍 Azure Storage Quota Report</h1>
...
$htmlContent += @"
        ...
        <div class="footer">
            <p>Generated by Azure Storage Quota Finder | PowerShell Script</p>
        </div>
    </div>
...
# Save HTML report
$htmlPath = Join-Path -Path (Get-Location) -ChildPath "StorageQuotaReport.html"
